...
if (isset($_GET['id'])) {
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            $updateQuery = "UPDATE Accounts SET U_type = '$newUType', UserName = '$newUserName', password = '$newPassword', Status = '$newAttribute' WHERE UserName = '$userId'";
            $updateResult = mysqli_query($connect, $updateQuery);

            if ($updateResult) {
                echo "User attributes updated successfully.";
            } else {
                echo "Error updating user attributes: " . mysqli_error($connect);
            }
        }
        ?>
